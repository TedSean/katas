...
c dec -10 if a >= 1
c inc -20 if c == 10`;
const data2 = `ev dec -705 if cag != 2
b dec -195 if ty >= -8
red dec -404 if v > -7
d inc -971 if k >= -9
k inc 303 if k > -8
...
const parse = (input) =>
  input.split('\n').map(instruction => {
    const [first, condition] = instruction.split(' if ');
    let [register, action, value] = first.split(' ');
    value = Number(value);
    let [condRegister, conditionOp, conditionValue] = condition.split(' ');
    conditionValue = Number(conditionValue);
    return {
      action: {register, action, value}, condition: {condRegister, conditionOp, conditionValue}
    }
  });


const checkCondition = (condition, registers) => {
  const {condRegister, conditionOp, conditionValue} = condition;
  switch (conditionOp) {
    case '>':
      return getRegValue(condRegister, registers) > conditionValue;
    case '<':
      return getRegValue(condRegister, registers) < conditionValue;
    case '>=':
      return getRegValue(condRegister, registers) >= conditionValue;
...
const performAction = (action, registers) => {
  switch (action.action) {
    case 'inc':
      registers[action.register] = getRegValue(action.register, registers) + action.value;
      break;
    case 'dec':
      registers[action.register] = getRegValue(action.register, registers) - action.value;
      break;
...
const getRegValue = (name, register) => {
...
const register = (input) => {
  let registers = {};
  let max = -100;
  parse(input).forEach(instruction => {
    const {condition, action} = instruction;
    if (checkCondition(condition, registers)) {
      performAction(action, registers);
    }
    const currentMax = Math.max(...Object.keys(registers).map(key => registers[key]));
