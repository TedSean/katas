...
const findCorrectPassPhrases = (input, passPhraseValidator) =>
  input
    .split('\n')
    .reduce((correctCount, passPhrase) =>
      passPhraseValidator(passPhrase) ? correctCount + 1 : correctCount, 0);

const isHaveDuplicates = (passPhrase) => {
  const words = passPhrase.split(' ');
  return words.every((value, index, array) => array.lastIndexOf(value) === index)
}

const isHaveAngrams = (passPhrase) =>
  isHaveDuplicates(passPhrase.split(' ')
    .map(word => word.split('')
      .sort()
      .join(''))
    .join(' ')
...
describe.only('entropy passPhrases', () => {
  it('should return', () => {
    ...
    expect(findCorrectPassPhrases(input, isHaveAngrams)).to.equal(450);
  });
});


const demoInput = `abcde fghij
abcde xyz ecdab`
